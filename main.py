"""
StoryQuest+++ (Arcade-safe single file)

- Uses only draw_lrbt_rectangle_filled/outline, draw_circle_* and draw_line
- No arcade.Camera, no set_viewport (so no API crashes on older/newer builds)
- All UI text uses cached arcade.Text (no draw_text spam)
- Enemies/Boss have HP bars; player HUD; perks; wave → boss
"""
...
ELITE_BODY  = arcade.color.LIGHT_RED_OCHRE
...
def dist(x1: float, y1: float, x2: float, y2: float) -> float:
    """Arcade-safe distance (older builds lack arcade.get_distance)."""
...
def perk_pool():
    ...
    pool.append(Perk("Dash Mastery", "Dash CD -25%, +20% i-frames.",
    ...
    pool.append(Perk("Pierce", "Bullets pierce one extra target.",   lambda p: setattr(p, "pierce", p.pierce + 1)))
    return pool
...
class Player(arcade.SpriteCircle):
    def __init__(self):
        ...
        self.shield     = 0  # absorbs 1 hit per stack
...
class Chaser(Enemy):
    ...
    def step(self, player: Player, dt):
        ...
        seek = 2.6 if self.slow_t<=0 else 1.4
        if self.elite: seek *= 1.2
...
class Bullet(arcade.SpriteCircle):
    def __init__(self, x, y, dx, dy, speed, color, owner: str, radius=3, pierce_left=0):
...
class Pickup(arcade.SpriteCircle):
    def __init__(self, x, y, kind: str):
        color = arcade.color.SKY_BLUE if kind=="shield" else arcade.color.SPRING_GREEN
...
class Boss(arcade.SpriteCircle):
    def __init__(self):
        ...
        self.max_hp = 240; self.hp = self.max_hp
...
def draw_lrbt_rect_outline_center(cx, cy, w, h, color, line_width=1):
    l = cx - w/2; r = cx + w/2; b = cy - h/2; t = cy + h/2
    arcade.draw_lrbt_rectangle_outline(l, r, b, t, color, line_width)
...
def draw_health_bar(x, y, width, height, hp, hp_max, edge_color=arcade.color.WHITE):
    ...
    if ratio > 0:
        fill_w = width*ratio
        l = x - width/2
        arcade.draw_lrbt_rectangle_filled(l, l+fill_w, y - (height-2)/2, y + (height-2)/2, HP_BAR_GREEN if ratio > 0.6 else HP_BAR_YELLOW if ratio > 0.3 else HP_BAR_RED)
    # edge
...
class MenuView(arcade.View):
    def __init__(self):
        ...
        self.subtitle = make_text("Boss fix • Cached text • Smarter AI • Perks • Pickups", 16, arcade.color.LIGHT_GRAY, "center")
        self.controls = make_text("Move: WASD  •  Shoot: SPACE / Left Click  •  Melee: Z  •  Dash: LSHIFT  •  Pause: ESC", 12, arcade.color.WHITE, "center")
...
class PerkDraftView(arcade.View):
    ...
    def on_draw(self):
        ...
        for i, perk in enumerate(self.options):
            ...
            # option panel (converted to lrbt)
            arcade.draw_lrbt_rectangle_filled(SCREEN_WIDTH/2 - 340, SCREEN_WIDTH/2 + 340, y-32, y+32, (0,0,0,140))
...
class GameOverView(arcade.View):
    def __init__(self, score: int, win: bool, seconds: float):
        ...
        self.title  = make_text("VICTORY!" if self.win else "DEFEAT",
                                40, arcade.color.LIGHT_GREEN if self.win else arcade.color.SALMON, "center")
...
class GameView(arcade.View):
    def __init__(self):
        super().__init__()
        # No camera/viewport transforms (to avoid API issues on your build)
        self.shake_t = 0.0
        self.flash_t = 0.0
    ...
    def on_draw(self):
        ...
        if self.paused:
            ...
            draw_text_shadowed(t1, cx, cy + 26)
            draw_text_shadowed(t2, cx, cy - 12)
    ...
    def _draw_hud(self):
        ...
        if filled>0:
            arcade.draw_lrbt_rectangle_filled(12, 12+filled, SCREEN_HEIGHT-62, SCREEN_HEIGHT-48, arcade.color.SPRING_BUD)
    ...
    def on_update(self, dt: float):
        ...
        self.fire_timer.update(dt); self.dash_timer.update(dt); self.melee_timer.update(dt)
    ...
    def _boss_logic(self, dt):
        ...
        if b.phase==1:
            if b.phase_timer>1.0:
                b.phase_timer=0.0
                self._boss_fire_aimed(b, speed=7.2, color=arcade.color.PURPLE)
            ...
            if random.random()<0.018 and len(self.enemy_list)<10:
                self.enemy_list.append(Chaser(random.randint(ARENA_MARGIN, SCREEN_WIDTH-ARENA_MARGIN), random.randint(SCREEN_HEIGHT//2, SCREEN_HEIGHT-ARENA_MARGIN-20)))
            if random.random()<0.012 and len(self.enemy_list)<12:
                self.enemy_list.append(Shooter(random.randint(ARENA_MARGIN, SCREEN_WIDTH-ARENA_MARGIN), random.randint(SCREEN_HEIGHT//2, SCREEN_HEIGHT-ARENA_MARGIN-20)))
        ...
        if self.player.has_spread:
            ...
            for i in range(SHOTGUN_PELLETS):
                ...
                self.bullets.append(Bullet(self.player.center_x, self.player.center_y,
                                            ...
                                            self.player.bullet_speed, BULLET_COLOR_PLAYER, "player", pierce_left=pierce_left))
        else:
